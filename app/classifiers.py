...
class Classifier:
    ...
    def __init__(self):
        ...
        self.cp_mapping = {
            ...
            "MULTICHAIN": {"ALL": self.cl_multichain_all},
            "Raydium": {
            ...
            "Tulip": {"Farms": self.cl_tulip_farms},
            "Francium": {"Farms": self.cl_francium_farms},
            "JUPITER": {"CATCHMENT": self.cl_jupiter_swap},
            "Mango": {"Account": self.cl_mango},
            "Magic Eden": {
                "Account-related (V2)": self.cl_magiceden_v2,
                "Account-related (V1)": self.cl_magiceden_v1,
            },
            "Serum": {"DEX (v3)": self.cl_serum_all},
            "Solanart": {"Account-related": self.cl_solanart},
            # 'TETHER': {
    ...
    def remove_liquidity(self, transaction, ignore_tokens=(), pool_type=None):
        return self.chain.pools.remove_liquidity(
    ...
    def compare(self, transaction):
        if (
            transaction.derived_data is not None
            and transaction.changed is not None
            and transaction.changed != "NEW"
        ):
            ...
            try:
                ...
                for transfer in transaction.transfers.values():
                    if transfer.treatment != transfer.derived_data["treatment"]:
                        transfer.changed["Tax treatment"] = [
                            transfer.derived_data["treatment"],
                            transfer.treatment,
                        ]
                    if transfer.vault_id != transfer.derived_data["vault_id"]:
                        transfer.changed["Vault/Loan ID"] = [
                            transfer.derived_data["vault_id"],
                            transfer.vault_id,
                        ]
            except:
                pass
    ...
    def classify(self, transaction):
        logtx = False
        if transaction.hash == transaction.chain.hif or (
            transaction.hash is not None and transaction.hash in transaction.chain.hif
        ):
        ...
        if transaction.derived_data is not None and transaction.changed is None:
            ...
            if dd["category"] is not None:
                transaction.type = Category(
                    dd["category"],
                    claim_reward=dd["claim"],
                    nft=dd["nft"],
                    certainty=dd["certainty"],
        ...
        if self.chain.is_upload:
            ...
            self_transfers = transaction.lookup(
                {"to_me": True, "from_me": True, "amount_non_zero": True}
            )
            minted = transaction.lookup(
                {"to_me": True, "fr": Classifier.NULL, "amount_non_zero": True}
            )
            burned = transaction.lookup(
                {"from_me": True, "to": Classifier.NULL, "amount_non_zero": True}
            ) + transaction.lookup(
                {"from_me": True, "to": Classifier.DEAD, "amount_non_zero": True}
            )
            ...
            unstaked = transaction.lookup(
                {
                    "to_me": True,
                    "amount_non_zero": True,
                    "what": self.chain.pools.receipt_token_list(),
                    "fr": self.chain.pools.pool_address_list(),
                }
            )
            from_pools = transaction.lookup(
            ...
            unvaulted = transaction.lookup(
                {
                    "to_me": True,
                    "amount_non_zero": True,
                    "what": self.chain.pools.input_token_list(Pool.VAULT),
                    "fr": self.chain.pools.pool_address_list(Pool.VAULT),
                }
            )
            ...
            minted_nfts = transaction.lookup(
            ...
            if (
                ...
                and transaction.interacted in self.address_to_cp_mapping
            ):
                cp_pair = self.address_to_cp_mapping[transaction.interacted]
                cp = cp_pair[0]
                transaction.counter_parties = {
                    transaction.interacted: [cp, None, None, None, transaction.interacted]
                }
            ...
            if cp_pair is not None and len(cp_pair) > 1:
                ...
                if current_sig is None:
                    current_sig = ""
                transaction.counter_parties[k][1] = transaction.counter_parties[k][2] = sig = (
                    cp_pair[1] + " " + current_sig
                )
            ...
            if err is None:
                ...
                if rv is not None:
                    ...
                    if (
                        ...
                        and "CATCHMENT" in specific_checkers
                    ):
                    ...
                    if len(types) > 1:
                        ...
                        for type in types:
                            certainty_mapping[
                                type.certainty + int(type.protocol is not None)
                            ].append(
                                type
                            )  # advantage given to identified protocol
                        ...
                        for type in types:
                            if (
                                type.category in [Category.WRAP, Category.UNWRAP]
                                and type.certainty == 10
                            ):
                                types_noswap = [type]
                                break
                ...
                if len(types) == 1:
                    ...
                    if (
                        ...
                        and transaction.type.certainty >= 0
                        and len(received_nonzero)
                        and len(sent_nonzero)
                    ):
        ...
        if logtx:
            log(
                ...
                "category",
                transaction.type,
                transaction.classification_certainty_level,
                combo,
                len(transaction.transfers),
            )
            log("tx amounts", transaction.amounts)
            for cat in transaction.categorized_transfers.keys():
                log(
                    "transfer type",
                    Transfer.name_map[cat],
                    len(transaction.categorized_transfers[cat]),
                )
    ...
    def check_self_transfers(self, transaction):
        ...
        if len(CT[Transfer.SELF]) > 0:
            ...
            if len(CT[Transfer.SELF]) == len(CT[Transfer.RECEIVED]) + len(CT[Transfer.SENT]):
                ...
                len(CT[Transfer.SELF]) == len(CT[Transfer.RECEIVED]) + len(CT[Transfer.SENT]) - 1
                and transaction.fee_transfer is not None
            ):
                certainty = 10
            ...
            clog(
                transaction,
                "check_self_transfers",
                len(CT[Transfer.SELF]),
                len(transaction.transfers),
                "self cat assigned",
                certainty,
            )
    ...
    def check_reward(self, transaction, sig):
        ...
        if hasattr(transaction, "reward_token"):
            ...
            if len(reward_transfers) > 0:
                ...
                if (
                    len(reward_transfers) == len(CT[Transfer.RECEIVED])
                    and len(CT[Transfer.SENT]) == 0
                ):
                    if transaction.type is None:
        ...
        if isinstance(transaction.type, Category):
            if not (
                ...
                in [
                    ...
                    Category.ADD_LIQUIDITY,
                    Category.REMOVE_LIQUIDITY,
                    Category.REMOVE_LIQUIDITY_NO_RECEIPT,
                    Category.ADD_LIQUIDITY_NO_RECEIPT,
                    Category.EXIT_VAULT,
                ]
            ):
                return
        ...
        if len(CT[Transfer.REWARDS_LP]) > 0:
            ...
            if (
                len(pools) > 1
            ):  # sometimes there's a 0-valued transfer specifying which LP token was used to stake
                lp_indicator = transaction.lookup(
                    {"from_me": True, "amount_non_zero": False, "type": 3}
                )
                ...
                if len(lp_indicator) == 1:
                    ...
                    pool_transfers = (
                        CT[Transfer.UNSTAKED_LP] + CT[Transfer.STAKED_LP] + CT[Transfer.UNVAULTED]
                    )
                    ...
                    if len(transferred_tokens) == 1:
                        ...
                        pools_by_token = self.chain.pools.pool_list(
                            "I", list(transferred_tokens)[0]
                        )
                        pools = pools.intersection(pools_by_token)
        ...
        # reward token is minted, not sent from pool
            ...
            and transaction.interacted is not None
            and not self.check_sig(sig, "mint")
            and not self.check_sig(sig, "borrow")
        ):
            max_certainty = 3
        ...
        # reward token is sent from a different address
            ...
            and (
                self.check_sig(sig, "claim")
                or self.check_sig(sig, "reward")
                or self.check_sig(sig, "withdraw")
                or self.check_sig(sig, "harvest")
            )
            ...
            interactor = transaction.interacted
            reward_transfers = (
                set(CT[Transfer.RECEIVED])
                - set(CT[Transfer.UNSTAKED_LP])
                - set(CT[Transfer.UNVAULTED])
            )
            if len(reward_transfers) > 0:
        ...
        if len(pools) >= 1:
            ...
            if (
                (pool.type in [Pool.STAKING, Pool.VAULT] or self.check_sig(sig, "reward"))
                or self.check_sig(sig, "claim")
                and rew.what not in pool.deposited
            ):
    ...
    def cl_deposit(self, transaction, sig):
        ...
        if transaction.hash == transaction.chain.hif:
            log(
                "dep check bridge",
                len(transaction.transfers),
                transfers[0].rate_found,
                len(CT[Transfer.NFT_IN]),
            )
        ...
        if (
            ...
            if (len(CT[Transfer.FROM_BRIDGE]) or len(CT[Transfer.RECEIVED]) == 1) and (
                ...
                for transfer in CT[Transfer.RECEIVED]:
                    if transaction.hash == transaction.chain.hif:
                        log("bridge 2")
                    match_found, match_certainty = self.find_outgoing_bridge_match(
            ...
            if not len(CT[Transfer.NFT_IN]):
                ...
                if (
                    ...
                    or transaction.chain.name == "Solana"
                ):
                    certainty = 5
    ...
    def cl_wrap_unwrap(self, transaction, sig):
        CT = transaction.categorized_transfers
        if (
            transaction.interacted == transaction.chain.wrapper
            ...
            if len(CT[Transfer.TO_BRIDGE]) or len(CT[Transfer.SENT]) == 1:
                for transfer in CT[Transfer.SENT]:
                    if transfer.amount != 0:
                        ...
                        self.outgoing_transfers[transaction.ts] = {
                            "chain": self.chain,
                            "transfer": transfer,
                            "matched": False,
    ...
    def cl_add(self, transaction, sig):
        ...
        if (
            ...
            if (
                ...
                if (
                    abs(transaction.amounts[CT[Transfer.MINTED][0].what])
                    == CT[Transfer.MINTED][0].amount
                ):
    ...
    def cl_remove(self, transaction, sig):
        ...
        if (
            ...
            if len(CT[Transfer.BURNED]) == 1:  # burned liquidity pool token
                if (
                    abs(transaction.amounts[CT[Transfer.BURNED][0].what])
                    == CT[Transfer.BURNED][0].amount
                ):
                    do_remove = True
    ...
    def cl_vault(self, transaction, sig):
        ...
        if (
            len(CT[Transfer.SENT])
            and self.check_sig(sig, "stake")
            and not self.check_sig(sig, "unstake")
        ):  # who am I to argue?
            self.add_liquidity(transaction)
        ...
        if (
            len(CT[Transfer.SENT]) == 1
            and len(transaction.transfers) <= 3
            and self.check_sig(sig, "deposit")
            and len(CT[Transfer.TO_BRIDGE]) == 0
        ):
            if transaction.hash == self.chain.hif:
                print("cl_vault XI1")
            self.add_liquidity(transaction)
    ...
    def cl_unvault(self, transaction, sig):
        ...
        if (
            ...
            if transaction.interacted:
                ...
                for transfer in CT[Transfer.RECEIVED]:
                    pools_for_tokens = pools_for_tokens.union(
                        self.chain.pools.matches(transfer.what, "I")
                    )
    ...
    def cl_spam(self, transaction, sig):
        ...
        clog(
            ...
            "spam? 0",
            transaction.interacted,
            len(received) == len(transaction.transfers),
            transaction.interacted is not None,
            not transaction.my_address(transaction.originator),
            transaction.interacted not in self.spam_data["previously_interacted"],
        )
        ...
        if (
            ...
            for tr in received:
                ...
                clog(
                    ...
                    self.chain.name,
                    tr.what,
                    transaction.user.current_tokens[self.chain.name][tr.to],
                )

                if tr.token_nft_id is not None:
                    if transaction.user.current_tokens is not None:
                        try:
                            floor = transaction.user.current_tokens[self.chain.name][tr.to][
                                tr.what
                            ]["rate"][1]
        ...
        if len(tokens) == 2:
            ...
            if len(tokens) == 1:
                ...
                transaction.protocol_note = (
                    "We are not sure about the vault ID on the " + tokens[0] + " transfer"
                )
    ...
    def find_outgoing_bridge_match(self, transaction, transfer, protocol=None):
        ...
        for outgoing_idx in range(0, len(self.outgoing_transfers)):
            ...
            if (
                ...
                and candidate.type not in [Transfer.ERC721, Transfer.ERC1155]
                and transfer.type not in [Transfer.ERC721, Transfer.ERC1155]
            ):
                if (
                    candidate.symbol == transfer.symbol
                    or "W" + candidate.symbol == transfer.symbol
                    ...
                    candidate.coingecko_id is not None
                    and transfer.coingecko_id is not None
                    and (
                        "bridged-" in candidate.coingecko_id
                        or "bridged-" in transfer.coingecko_id
                        or "wrapped-" in candidate.coingecko_id
                        or "wrapped-" in transfer.coingecko_id
                    )
                    and (
                        (
                            transfer.rate != 0
                            and transfer.rate is not None
                            and candidate.rate != 0
                            and candidate.rate is not None
                            and abs(transfer.rate / candidate.rate - 1) < 0.03
                        )
                        or candidate.coingecko_id in transfer.coingecko_id
                ...
                if proceed:
                    ...
                    if (
                        transfer.amount <= candidate.amount
                        and transfer.amount > 0.97 * candidate.amount
                    ):
                        ...
                        if (
                            transfer.amount != candidate.amount
                            or transfer.coingecko_id != candidate.coingecko_id
                        ):
                            ...
                            transfer.treatment = "ignore"
                        candidate_tx.type = Category(
                            Category.WITHDRAW_TO_BRIDGE, certainty=certainty, protocol=protocol
                        )
    ...
    def cl_multichain_all(self, transaction, sig):
        ...
        if len(CT[Transfer.SENT]) == 1 and len(CT[Transfer.RECEIVED]) == 0:
            ...
            match_found, match_certainty = self.find_outgoing_bridge_match(
                transaction, candidate, protocol="MULTICHAIN"
            )
            ...
            if mint.what == burn.what and mint.amount == burn.amount:
                for transfer in transaction.transfers.values():
                    ...
                    if (
                        transfer.from_me
                        and transfer.to != Classifier.NULL
                        and transfer.to != "network"
    ...
    def cl_tulip_farms(self, transaction, sig):
        ...
        if (
            len(CT[Transfer.RECEIVED]) == 1 and len(CT[Transfer.SENT]) == 0
        ):  # and not self.check_sig(sig,'repay'):
        ...
        if (
            len(CT[Transfer.RECEIVED]) in [1, 2] and len(CT[Transfer.SENT]) == 0
        ):  # and self.check_sig(sig,'repay'):
            for tr in CT[Transfer.RECEIVED]:
    ...
    def cl_mango(self, transaction, sig):
        ...
        if self.check_sig(sig, "transfer") and (
            len(CT[Transfer.RECEIVED]) == 0
    ...
    def cl_magiceden_v1(self, transaction, sig):
        ...
        if len(CT[Transfer.SENT]) == 0:
            for tr in CT[Transfer.RECEIVED]:
                ...
                tr.treatment = "withdraw"
            return Category(
                Category.REMOVE_LIQUIDITY_NO_RECEIPT, certainty=3, protocol="MAGIC EDEN"
            )
    ...
    def cl_magiceden_v2(self, transaction, sig):
        ...
        if len(CT[Transfer.RECEIVED]) == 1 and len(CT[Transfer.SENT]) == 0:
            return Category(
                Category.REMOVE_LIQUIDITY_NO_RECEIPT, certainty=5, protocol="MAGIC EDEN"
            )
    ...
    def cl_serum_all(self, transaction, sig):
        ...
        if len(CT[Transfer.RECEIVED]) in [1, 2] and len(CT[Transfer.SENT]) == 0:
            ...
            if (
                vault_id in self.prior_withdraw
                and self.prior_withdraw[vault_id].type.category == Category.EXIT_VAULT
            ):
    ...
    def process_classification(self, transaction):
        ...
        if cat == Category.FEE:
            ...
            for t in transaction.transfers.values():
                if (
                    t.synthetic == Transfer.FEE
                ):  # these are not in CT[Transfer.SENT] -- they are not mapped
                    set_treatment(t, "fee")
            ...
            if len(received_nonzero) == 0:
                ...
                if function == "send" and len(sent_nonzero) == 1:
                    self.outgoing_transfers[transaction.ts] = {
                        ...
                        "transfer": sent_nonzero[0],
                        "matched": False,
                        "transaction": transaction,
                    }

        elif function in ["receive", "fiat-deposit", "gift"] or (
            function == "trade" and len(bought) > 0
        ):
            if len(sent_nonzero) == 0:
                ...
                if function == "receive" and len(received_nonzero) == 1:
                    ...
                    match_found, match_certainty = self.find_outgoing_bridge_match(
                        transaction, received_nonzero[0]
                    )
                    if match_found:
