...
class Twelve:
    ...
    def init_rates(self):
        ...
        for symbol in Twelve.FIAT_SYMBOLS:
            ...
            rows = self.db.select(
                "SELECT timestamp,rate FROM fiat_rates WHERE currency='" + symbol + "'"
            )
            for row in rows:
    ...
    def download_all_rates(self, do_wait=False):
        ...
        if len(all_db_writes) > 0:
            ...
            for entry in all_db_writes:
                self.db.insert_kw(
                    "fiat_rates", currency=entry[0], timestamp=entry[1], rate=entry[2]
                )
            self.db.commit()
        self.db.disconnect()
    ...
    def download_rates(self, symbol):
        ...
        while latest_available < t - 3 * 24 * 3600:
            ...
            url = (
                "https://api.twelvedata.com/time_series?symbol=USD/"
                + symbol
                + "&interval=1h&outputsize=5000&timezone=UTC&start_date="
                + start_date
                + "&end_date="
                + end_date
                + "&order=ASC&apikey="
