...
class Pair:
    def __init__(
        self, contract, chain_name, exchange, base_token, base_symbol, quote_token, quote_symbol
    ):
    ...
    def get_cmc_id(self):
        ...
        url = (
            "https://api.coinmarketcap.com/dexer/v3/dexer/pair-info?dexer-platform-name="
            + chain_name
            + "&address="
            + self.contract
        )
    ...
    def download_dexscreener_rates(self, latest_available=None):
        ...
        url = (
            "https://io.dexscreener.com/dex/chart/amm/uniswap/bars/"
            + dexscreener_name
            + "/"
            + self.contract
            + "?from="
        ...
        try:
            ...
            for entry in data:
                ...
                try:
                    ...
                    liquidityUpdatedAt_ts = int(
                        datetime.datetime.strptime(liquidityUpdatedAt[:19], "%Y-%m-%dT%H:%M:%S")
                ...
                try:
                    if (
                        entry["id"]["token"].lower() != contract.lower()
                        and entry["id"]["tokenRef"] != contract.lower()
                    ):
                        continue
                except:
                    continue
                ...
                creation_ts = int(
                    datetime.datetime.strptime(creation_time[:19], "%Y-%m-%dT%H:%M:%S")
                    .replace(tzinfo=datetime.timezone.utc)
                    .timestamp()
                )
